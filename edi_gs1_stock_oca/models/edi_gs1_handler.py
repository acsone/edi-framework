...
class GS1GenerateInstructionMixin(models.AbstractModel):
    ...
    def generate(self, exchange_record):
        ...
        # Pre-render the business header and inject it (via ``t-out``) at the
        # right position. We cannot ``t-call`` it inline because the
        # ``nswrapper`` purge moves its content to the end of the document,
        # which breaks the XSD element ordering (the header must come first).
